...
function Originate(idclicamptel)
{
	
	var spantimeroculto = "<span hidden class='class"+idclicamptel+"' ></span>";	
	document.getElementById(idclicamptel).innerHTML = "Discando..."+"<span class='class"+idclicamptel+"' ></span>";	
	/*Inicia timer de 40 segundos - timeout da discagem*/
	timer(
    41000, // milliseconds
    function(timeleft) { // called every step to update the visible countdown
        //document.getElementById('timer').innerHTML = timeleft+"(s)";
		$(".class"+idclicamptel).html(timeleft+"(s)");
    },
	...
	var baseurl = window.location.origin + window.location.pathname + "/discador_manual/";	
	var jqxhr = $.post( baseurl+ idclicamptel, function( data ) {
	DesativaBotoes();
})
  .done(function(data) {
	...
	switch(idstatus) {
		case 4:  // nao atendeu		
			...
			document.getElementById(idclicamptel).innerHTML = retorno+spantimeroculto;
			SalvaChamada(idstatus, idclicamptel, retorno);
			AtivaBotoes();			
			var curr = $("#tr" + idclicamptel).next('tr').attr('id');
  ...
  .fail(function(data) {
     document.getElementById(idclicamptel).innerHTML = data ;
	 //SalvaChamada(idstatus, idclicamptel, retorno);
	AtivaBotoes();
	var curr = $("#tr" + idclicamptel).next('tr').attr('id');
	var res = curr.replace("tr", "bt");
  ...
  .always(function(data) {
   // document.getElementById(idclicamptel).innerHTML = "alwais" ;
...
//Murilo - 30/11/2016
function SalvaChamada(_idstatus, _idclicamptel, _retorno)
{
    var x;
    x = new XMLHttpRequest();
    var url = window.location.origin + window.location.pathname + "/historico_ligacoes";
    var params = "retorno=" + _retorno + "&idclicamptel=" + _idclicamptel + "&idstatus=" + _idstatus;
